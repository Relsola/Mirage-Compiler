...
internal Token *skip_line(Token *tok)
{
    if (tok->at_bol) {
        return tok;
    }
...
internal Token *new_eof(Token *tok)
{
...
internal Hideset *hideset_union(Hideset *hs1, Hideset *hs2)
{
    Hideset head = {};
    Hideset *cur = &head;
...
internal bool hideset_contains(Hideset *hs, char *s, int len)
{
    for (; hs; hs = hs->next) {
...
internal Hideset *hideset_intersection(Hideset *hs1, Hideset *hs2)
{
    Hideset head = {};
    Hideset *cur = &head;
...
internal Token *add_hideset(Token *tok, Hideset *hs)
{
    Token head = {};
    Token *cur = &head;
...
internal Token *append(Token *tok1, Token *tok2)
{
    if (tok1->kind == TK_EOF) {
        return tok2;
    }
...
internal Token *skip_cond_incl2(Token *tok)
{
...
internal Token *skip_cond_incl(Token *tok)
{
    while (tok->kind != TK_EOF) {
        if (is_hash(tok) && (equal(tok->next, "if") || equal(tok->next, "ifdef") || equal(tok->next, "ifndef"))) {
            tok = skip_cond_incl2(tok->next->next);
...
internal char *quote_string(char *str)
{
    int bufsize = 3;
    for (int i = 0; str[i]; i++) {
        if (str[i] == '\\' || str[i] == '"') {
...
internal Token *new_str_token(char *str, Token *tmpl) {
...
internal Token *copy_line(Token **rest, Token *tok)
{
    Token head = {};
    Token *cur = &head;
...
internal MacroParam *read_macro_params(Token **rest, Token *tok, bool *is_variadic)
{
    MacroParam head = {};
    MacroParam *cur = &head;
...
internal void read_macro_definition(Token **rest, Token *tok)
{
    if (tok->kind != TK_IDENT) {
...
internal MacroArg *read_macro_arg_one(Token **rest, Token *tok, bool read_rest)
{
    Token head = {};
    Token *cur = &head;
    int level = 0;
...
internal MacroArg *read_macro_args(Token **rest, Token *tok, MacroParam *params, bool is_variadic)
{
    Token *start = tok;
    tok = tok->next->next;
...
internal MacroArg *find_arg(MacroArg *args, Token *tok)
{
...
internal char *join_tokens(Token *tok, Token *end)
{
    ...
    for (Token *t = tok; t != end && t->kind != TK_EOF; t = t->next) {
        ...
        strncpy_s(buf + pos, len - pos, t->loc, t->len);
        pos += t->len;
    }
    buf[pos] = '\0';
    return buf;
...
internal Token *stringize(Token *hash, Token *arg)
{
...
internal Token *paste(Token *lhs, Token *rhs)
{
...
internal Token *subst(Token *tok, MacroArg *args)
{
    Token head = {};
    Token *cur = &head;
...
internal bool expand_macro(Token **rest, Token *tok)
{
...
internal char *format_date(struct tm *tm)
{
    local_persist char mon[][4] = {
...
internal char *format_time(struct tm *tm)
{
...
void init_macros(void)
{
    ...
    define_macro("__DATE__", format_date(&tm_buf));
    define_macro("__TIME__", format_time(&tm_buf));
}

// Concatenate adjacent string literals into a single string literal
// as per the C spec.
internal void join_adjacent_string_literals(Token *tok1)
{
    while (tok1->kind != TK_EOF) {
        ...
        char *buf = arena_push(tok1->ty->base->size, len);
